...
Saved: qam64_35.png
```
![Simple visualization generated by `ria view`](/ria/toolkit-pictures/qam64_35.png)
<div style="font-size: 0.85rem; text-align: center; opacity: 0.75; margin-top: 0.5rem;">
  Example output from <code>ria view qam64_35.npy --type simple</code>.
</div>
...
Saving to: sample_recording3_annotated.npy
  ✓ Saved
```
To visually inspect the detected annotations:

```bash
ria view sample_recording3_annotated.npy --type annotations
```

![Energy annotation output](/ria/toolkit-pictures/sample_recording3_annotated.png)

<small>
Output from <code>ria annotate energy</code>. The blue annotation boxes indicate signal regions detected above the estimated noise floor.
</small>
...
To visually inspect the detected annotations:
```bash 
ria view sample_recording3_threshold.npy --type annotations
```
![Threshold annotation output](/ria/toolkit-pictures/out.png)

<small>
Output from <code>ria annotate threshold</code>. Signal regions exceeding the configured magnitude threshold are highlighted as annotations.
</small>
...
```bash
ria view sample_recording3_cusum.npy --type annotations
```

![CUSUM annotation output](/ria/toolkit-pictures/sample_recording3_cusum.png)

<small>
Output from <code>ria annotate cusum</code>. Annotation boundaries are generated from detected changes in signal behavior, segmenting the recording into distinct regimes.
</small>
...
To inspect the generated signal:

```bash
ria view qam16.npy --type simple --constellation
```

![QAM16 visualization](/ria/toolkit-pictures/qam16.png)

<small>
The constellation diagram (bottom left) shows the 16 symbol locations in the I/Q plane, while the spectrogram, IQ sample plot, and power spectral density provide additional insight into the signal's time- and frequency-domain characteristics.
</small>
---
